...
import { propertyData } from "../../../../types/property/propertyData";
...
const PropertyCard: React.FC<PropertyCardProps> = ({ property, viewMode }) => {  
  ...
  return (
    <div
      ...
      data-aos="fade-up"
    >
      <Link href={`/properties/properties-list/${property.slug || property.id}`} className={`group ${viewMode=="list" && 'flex' }`}>
        <div className={`relative ${viewMode=="list" && 'w-[30%]'}`}>
          <div className={`imageContainer h-[250px] w-full ${viewMode =="list" && 'h-full md:h-52'}`}>
            {property.image && typeof property.image === 'object' && property.image.url && (
              <Image
